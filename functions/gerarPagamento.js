        ...
        const interval = setInterval(async () => {
            ...
            if (tentativas >= 8 || pagamentoStatus === 'approved') {
                ...
                if (pagamentoStatus === 'approved') {
                    interaction.guild.members.cache.get(carrinhoDados.user_id).roles.add(`${config.idcargoCliente}`).catch(err => { /* console.log(`Uma pergunta... O cargo ${config.idcargoCliente.replace("", "\"Cliente\"")} existe no config.json?`) */ });
