...
client.once("ready", async () => {
  console.log(`🎄 » [APLICAÇÃO] ${client.user.username} Online.`);
});
...
client.on("messageCreate", async (message) => {
  ...
  if (timestamps && cooldown.has(message.author.id)) {
    ...
    let embedcd = new Discord.MessageEmbed()
      .setFooter({ text: "∙ discord.gg/shinestore ©" })
      .setAuthor({ name: `ShineStore`, iconURL: config.author })
      .setColor(config.color)
      .setDescription(`Você deve **aguardar** alguns segundos para usar um comando denovo.`);
